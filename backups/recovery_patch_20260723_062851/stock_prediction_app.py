...
def _render_stock_form():
    ...
    with st.form("stock_form", clear_on_submit=False):
        ...
        allow_provider_truncation = st.checkbox(
            "Allow provider-constrained run (if requested date range unavailable)",
            value=False,
            help=(
                "If unchecked (default): run is BLOCKED when the provider cannot satisfy "
                "your requested historical context start date. "
                "If checked: run proceeds using available provider data, clearly labeled as PROVIDER-CONSTRAINED."
            ),
        )
    ...
    if submitted:
        ...
        _run_all(
            ...
            run_id=run_id,
            price_basis=price_basis,
            allow_provider_truncation=allow_provider_truncation,
        )
...
def _run_all(
    ...
    if _effective_ctx_start > ctx_start:
        ...
        if not allow_provider_truncation:
            # STRICT MODE: block the run — user must explicitly allow truncation
            _block_msg = (
                f"RUN BLOCKED — REQUESTED DATE RANGE NOT AVAILABLE\n\n"
                ...
                f"Accuracy Saved: NO\n\n"
                f"Reason: Provider cannot satisfy the requested historical context window. "
                f"Check 'Allow provider-constrained run' below the form to proceed with available data."
            )
            ...
            # PROVIDER-CONSTRAINED MODE: warn but allow
            _run_ctx["input_binding_warning"]  = (
                f"PROVIDER-CONSTRAINED RUN — You requested context from {ctx_start} but provider "
                f"only has data from {_effective_ctx_start} ({_date_gap_days} days short). "
                f"Proceeding with {_effective_ctx_start} as effective start (provider-constrained run). "
                f"This is NOT equivalent to a full {ctx_start} context window validation."
            )
